        ...
        return this.each(function () {
            ...
            if (options.displaytype == 'select') {
                ...
                if (options.required == false) {
                    el.push('<option value=""'+(value==""?" selected":"")+'></option>');
                   // flagDefault = true;
                }
               ...
               // if (options.required || options.isDefault) {
                ...
                $(options.items).each(function (i, n) {
                    ...
                    console.log(i)
                    if (!options.itemTmpl) {
                        el.push('<label class="checkbox-inline"><input type="radio" data-sourceid="' + (self.attr("id") || _thisid) + '" name="' + (name + +new Date().getTime()) + '" value="' + v + '"' + selected + '>' + text + '</label>');
                    } else {
                    ...
                    myTool1: {
                        ...
                        onclick: function (a, b, d, event) {
                            ...
                            if (target) {
                                ...
                                location.href = ORG_SERVERURL + '/entity/list?queryid=' + queryid;//xms没有entityname会有问题
                            }
                    ...
                    myTool1: {
                        ...
                        onclick: function (a, b, d, event) {
                            ...
                            if (target) {
                                var par = $(target).parents('.xms-chart');
                                var queryid = par.attr('data-queryid');
                                location.href = ORG_SERVERURL + '/entity/list?queryid=' + queryid;
                            }
...
; (function ($, root) {
    "use strict"
    var dTree = root.dTree;
    var setting = {
        datas: [],
        target: document.body,
        checkLayer: null,//只可以选择的层级
        parentid: "0",//根节点的 pid
        dataFilter: null,
        checkedHandler: function () { }//选中后的处理
    }
    
    $.fn.dropdownTree = function (opts) {
        opts = $.extend({}, setting, opts);
        $.template("xmsTreeTmpl", '<ul class="xms-dt-list"><li class="xms-dt-header" layer="${layer}" data-parentid="${parentid}" data-id="${id}">{{if nodes.length}} <a class="xms-dt-title xms-dt-ctrl"><em class="glyphicon glyphicon-plus"></em><span>${title}</span></a> {{tmpl(dTree.getFolderItems(nodes,id,"pid")) "#xmsTreeTmpl" }} {{else}} <a class="xms-dt-title xms-dt-ctrl"><em class="glyphicon"></em><span>${title}</span></a> {{tmpl(dTree.getFolderItems(nodes,id,"pid")) "#xmsTreeItemTmpl" }} {{/if}}</li></ul>');
        $.template("xmsTreeItemTmpl", '<li class="xms-dt-header" data-parentid="${parentid}" data-id="${id}"><a class="xms-dt-title xms-dt-ctrl"><em class="glyphicon glyphicon-minus"></em><span>${title}</span></a></li>');
        return this.each(function () {
            var datas = opts.datas, renderDatas = datas, $that = $(this)
            if (typeof opts.dataFilter === "function") {
                renderDatas = opts.dataFilter(datas);
            }
            $that.trigger("xmsDropDownTree", function (data) {
                renderTree(data);
            });
            renderTree(renderDatas);
            function renderTree(renderData) {
                var dtree = new dTree();
                var treeHead = $('<ul class="xms-dt-list"></ul>');
                var dataCacheDom = $('<input type="hidden" value="" />');
                var treeList = [];
                $(document.body).append(dataCacheDom);

                for (var i = 0, len = renderData.length; i < len; i++) {
                    var item = renderData[i], id = item.id, pid = item.parentid, title = item.title;
                    dtree.addLeaf(id, pid, title);//创建树节点
                }
                var treeDom = $.tmpl("xmsTreeTmpl", dtree.root);
                //var treeDom = $("#xmsTreeTmpl").tmpl(dtree.root);
                treeDom.appendTo();

                //打开关闭事件
                $(treeDom).on("click", ".xms-dt-ctrl>em", function () {
                    var $this = $that, parent = $this.parent().parent(), isactive = parent.attr("isopen");
                    if (isactive == "1") {
                        parent.attr("isopen", "0");
                        parent.removeClass("open");
                        $this.addClass("glyphicon-plus").removeClass("glyphicon-minus");
                    } else {
                        parent.attr("isopen", "1");
                ...
                //选择事件
                $(treeDom).on("click", ".xms-dt-ctrl>span", function () {
                    var $this = $that, parent = $this.parent().parent(), isactive = parent.attr("isactive");
                    if (opts.checkLayer && parent.attr("layer") != opts.checkLayer) return false;
                    $(treeDom).find(".xms-dt-header").removeClass("active")
                    parent.addClass("active");
                    opts.checkedHandler(parent);
                });
            }
        });
...
(function () {
    ...
    function uploadFile(obj, txshow, value, opts,e) {
        ...
        setTimeout(function () { 
            ...
            //$file.get(0).addEventListener('onpropertychange',filechange,false);
            //.on('onpropertychange',filechange);
        }, 500);
    }
    var settings = {
        uploadEnd: null,
        ...
        return this.each(function () {
            ...
            if (typeof (FileReader) === 'undefined') {
                ...
                $(this).parent().off('click').on('click', function (e) {
                    uploadFile($(this).children(), txshow, value, opts,e);
                });
            }
...
(function (root) {
    ...
    function delScriptByStr(arr, str) {
        if (typeof arr === "string") {
            var reg = '\<script+\\ssrc\\=\\"[\\w\\/\\-\\.]*' + arr + '+[\\?\\=\\s\\w\\/\\.]*\"+\\>+\\<\\/script\\>+';
            console.log(reg);
            var regobj = new RegExp(reg, "igm");
            console.log(str.match(regobj));
            str = str.replace(regobj, "");
        } else {
            for (var i = 0, len = arr.length; i < len; i++) {
                var reg = '\<script+\\ssrc\\=\\"[\\w\\/\\-\\.]*' + arr[i] + '+[\\?\\=\\s\\w\\/\\.]*\"+\\>+\\<\\/script\\>+';
                console.log(reg);
                var regobj = new RegExp(reg, "igm");
                console.log(str.match(regobj));
                str = str.replace(regobj, "");
    ...
    root.delScriptByStr = delScriptByStr;
})(window);
...
;(function (root, $, un) {
    ...
    function autoSearcher(obj,opts,datas) {
        ...
        this.opts = opts;
        this.datas = datas;
        this.vInput = $('<input class="xms-mutilauto-input form-control" type="text" />');
...
; (function (root, $) {
    ...
    function searchType(obj, opts) {
        this.box = $(obj);
        this.opts = opts;
        this.input = $('<input type="text" class="form-control" class="searchtype-input" />');
        this.searchBtn = $('<button class="btn btn-primary btn-sm"><span class="glyphicon glyphicon-search"></span></button>');
        this.wrap = $('<div class="searchtype-wrap"></div>');
    }
    searchType.prototype.init = function () {
        this.box.hide();
        this.box.after(this.wrap);
        this.wrap.append(this.input, this.searchBtn);

    }
    searchType.prototype.bindEvent = function () {
    ...
    $.fn.searchType = function (opts) {
        opts = $.extend({}, defaults, opts);
        this.each(function () {
            new searchType(this, opts);
        });
    }
})(window, jQuery);
...
(function () {
    var defaults = {
        ...
        , offsetTop: 0
        , offsetLeft: 0
        , colOffset: [0, 0, 0, 0]//left,top,width,height
        , headOffset: [0, 0, 0, 0]//left,top,width,height
        , maxLength: 1
        , forzenHandle: null
        , unforzenHandle: null
        , scrollElement: null
    ...
    function getTh($tr, index) {
        var res = null;
        $tr.find('th').each(function (key, item) {
            if (key == index) {
                res = item;
                return false;
            }
    ...
    function createthControlEle(th) {

    }

    function _showIndex(table, index) {
        var $table = $(table);
        var $thead = $table.find('>thead');
        var $tbody = $table.find('>tbody');
        if (!$.isArray(index)) { index = [index] };
        var sumWidth = 0;
        $thead.find('>tr>th').each(function (key, item) {
            if (~$.inArray(key, index)) {
                $(item).show();
        ...
        if (sumWidth >= 0) {
            ...
            $table.parent().width(sumWidth);
        }
    }
    function _hideIndex(table, index) {
        var $table = $(table);
        var $thead = $table.find('>thead');
        var $tbody = $table.find('>tbody');
        if (!$.isArray(index)) { index = [index] };
    ...
    function setShow(ele) {
        $(ele).css({ visibility: 'initial' });
    }
    function forzenTable(obj, opts) {
        this.dom = $(obj);
        this.opts = opts;
        this.clone = this.dom.clone(true);
        this.target = this.opts.target ? $(this.opts.target) : this.dom.parent();
        this.scrollElement = this.opts.scrollElement ? $(this.opts.scrollElement) : this.dom.parent();
        this.targetWidth = this.target.outerWidth();
        this.position = this.opts.position;
        this.__supter = null;
        //头部冻结 
        if (this.opts.fhead) {
            this.$head_wrap = $('<div class="fozentable-headerwrap"></div>');
            this.$headtable_clone = this.dom.clone(true);
            this.$headtable_clone.attr('id', 'forzontablehead_' + ((Math.random() * 50000) >> 0));
            this.$thead = this.$headtable_clone.find('thead');
            this.$headtable_clone.find('tbody').hide();
            this.target.append(this.$head_wrap);
            this.$head_wrap.append(this.$headtable_clone)
            this.$head_wrap.css({ 'position': 'absolute', 'z-index': '1', 'overflow': 'hidden', top: 0 + this.opts.headOffset[1], 'margin': '0 1px', 'height': this.dom.find('thead').height() + this.opts.headOffset[3], });
            if (this.opts.headOffset[2] > 0) {
                this.$head_wrap.css('width', this.opts.headOffset[2]);
            }
            this.$headtable_clone.css({ 'background': '#fff' });
            if (this.scrollElement.width() > 0) {
                this.$head_wrap.width(this.scrollElement.width());
            }
            this.$headtable_clone.width(this.dom.width())
            this.dom.children('thead').css('visibility', 'hidden');
            this.opts.resetWidth && $.fn.tableHdResize && this.$headtable_clone.tableHdResize({ resetTableWidth: false, linkTable: this.dom });
        }
        ...
        this.unForzenList = []
        if (this.opts.fcol) {
            this.wrap = $('<div class="forzontable-wrap"></div>');
            this.wrap.css({ position: 'absolute', 'z-index': '1', overflow: 'hidden', top: 0 + this.opts.colOffset[1], margin: '0px 1px', height: this.dom.parent().height() + this.opts.colOffset[3] });
            this.wrap.css(this.position, this.opts.colOffset[0]).css('border-' + (this.position == 'left' ? 'right' : 'left'), '1px solid #ddd');
            this.target.append(this.wrap);
            this.wrap.append(this.clone);
            this.dom.addClass('forzontalbe');
            this.clone.addClass('forzontalbe-clone');
            this.clone.css({ 'table-layout': 'initial', 'background-color': 'white' });
            this.clone.attr('id', 'forzontable_' + ((Math.random() * 50000) >> 0));
            this.dom_thead = this.dom.find('thead');
            this.dom_tbody = this.dom.find('tbody');
            this.wrap.hide();
            this.clone_thead = this.clone.find('thead');
            this.clone_thead.find('input[type="hidden"]').remove();
            this.clone_tbody = this.clone.find('tbody');
            this.clone_thead.css('visibility', 'initial');



            this.opts.fhead && setShow(this.clone_thead);
            this.opts.fcol && setShow(this.clone_tbody);
        }
        this.init();
    }

    forzenTable.prototype.init = function () {
        var self = this;
        //初始化表格头部，添加可控制列冻结的启动按钮
        if (!this.opts.fhead) {
            this.opts.fcol && this.dom_thead.find('th').each(function (key, item) {
                if (~$.inArray(key, self.opts.disabled)) { return true; }//忽略改列冻结
                if (self.opts.enable.length > 0) {
                    if (~$.inArray(key, self.opts.enable)) {
                        var $temp = $('<div data-index="' + key + '" title="冻结该列" class="forzontable-th glyphicon glyphicon-pushpin"></div>');
                        self.controlList.push($temp);
                        $(this).append($temp);
                    }
                } else {
                    var $temp = $('<div data-index="' + key + '" title="冻结该列" class="forzontable-th glyphicon glyphicon-pushpin"></div>');
            ...
            this.opts.fcol && this.clone_thead.find('th').each(function (key, item) {
                if (~$.inArray(key, self.opts.disabled)) { return true; }
                if (self.opts.enable.length > 0) {
                    if (~$.inArray(key, self.opts.enable)) {
                        var $temp = $('<div data-index="' + key + '" title="解冻该列" class="forzontable-th active glyphicon glyphicon-pushpin"></div>');
                        self.unForzenList.push($temp);
                        $(this).append($temp);
                    }
                } else {
                    var $temp = $('<div data-index="' + key + '" title="解冻该列" class="forzontable-th active glyphicon glyphicon-pushpin"></div>');
            ...
            this.opts.fcol && this.$thead.find('th').each(function (key, item) {
                if (~$.inArray(key, self.opts.disabled)) { return true; }//忽略改列冻结
                if (self.opts.enable.length > 0) {
                    if (~$.inArray(key, self.opts.enable)) {
                        var $temp = $('<div data-index="' + key + '" title="冻结该列" class="forzontable-th glyphicon glyphicon-pushpin"></div>');
            ...
            if (this.opts.colForzonHead) {
                self.clone.forzenTable({
                    fcol: false,
                    resetWidth: true
                    , headOffset: [0, this.opts.headOffset[1] + 2, 0, this.dom.find('thead').height()]
                    , target: this.target
                    , colForzonHead: false
                });
                self.clone.data().forzenTable.__super = self;
                self.clone.forzenTable('_bindEvent');
            }
            // self.$headtable_clone.forzenTable({fcol:false});
        }
    ...
    forzenTable.prototype.forzenHead = function () {

    }

    forzenTable.prototype.showHeadIndex = function (index) {
        _showIndex(this.$headtable_clone, index);
    }

    forzenTable.prototype.hideHeadIndex = function (index) {
        _hideIndex(this.$headtable_clone, index);
    }

    forzenTable.prototype.forzenCol = function (opts) {
        if (!opts || !opts.type) return false;
        if (opts.type == 'show') {
            this._showCol(opts.index);
        } else if (opts.type == 'hide') {
            this._hideCol(opts.index);
        }
    }
    //show为冻结，hide为不冻结
    forzenTable.prototype._showCol = function (index) {
        var self = this;
        if (~$.inArray(index, this.forzonList)) {
            return false;
        }
        if (this.forzonList.length >= this.opts.maxLength) {
            return false;
        }
        this.forzonList.push(index);
        this.wrap.show();
        self.clone.show();
        self.clone_tbody.find('tr').find('td').hide();
        self.clone_thead.find('tr').find('th').hide();
        var sumWidth = 0;
        this.clone.css({ 'table-layout': 'initial' });
        $.each(this.forzonList, function (key, item) {
            //设置显示和宽度
            var activetd = self.clone_tbody.find('tr').find('td:eq(' + item + ')');
            var dom_activetd = self.dom_tbody.find('tr').find('td:eq(' + item + ')');
            var domTh = getTh(self.dom_thead.find('tr'), item);
            var cloneTh = getTh(self.clone_thead.find('tr'), item);
            activetd.show();
            var _width = $(domTh).outerWidth();
            $(cloneTh).show().width(_width);
            sumWidth += _width;
            activetd.width(_width);
            if (self.opts.isHide) {
                dom_activetd.hide();
                $(domTh).hide();
                if (self.opts.fhead) {
                    var headerTh = getTh(self.$thead.find('tr'), item);
                    $(headerTh).hide();
                }
            }
        });
        this.clone.css({ 'table-layout': 'fixed' });
        //self.clone.css({ 'table-layout': 'fixed'});
        self.wrap.width(sumWidth).show();

        self.dom.parent().css('padding-' + this.position, sumWidth);
        if (self.opts.fhead) {
            self.$headtable_clone.width(self.targetWidth + sumWidth);
            self.$head_wrap.css('padding-' + this.position, sumWidth)
        }
        self.clone.width(sumWidth);

        self.dom.trigger('forzen.show', { obj: self, sumWidth: sumWidth });
        self.opts.forzenHandle && self.opts.forzenHandle(self);
        if (this.opts.colForzonHead) {
            self.clone.forzenTable('showHeadIndex', this.forzonList);
        }
    }

    forzenTable.prototype._hideCol = function (index) {
        var self = this;
        var _index = $.inArray(index, this.forzonList);
        if (!~_index) {
            return false;
        }

        this.forzonList.splice(_index, 1);
        self.clone.show();
        self.clone_tbody.find('tr').find('td').hide();
        self.clone_thead.find('tr').find('th').hide();
        var dom_showtd = self.dom_tbody.find('tr').find('td:eq(' + index + ')');
        var domshowTh = getTh(self.dom_thead.find('tr'), index);

        if (self.opts.isHide) {
            dom_showtd.show();
            $(domshowTh).show();
            if (self.opts.fhead) {
                var headershowTh = getTh(self.$thead.find('tr'), index);
                $(headershowTh).show();
            }
        }
        var sumWidth = 0;
        ...
        //self.clone.css({ 'table-layout': 'fixed'});
        self.wrap.width(sumWidth);


        if (this.forzonList.length == 0) {
            self.wrap.hide();
        }
        self.dom.parent().css('padding-' + this.position, sumWidth);
        if (self.opts.fhead) {
        ...
        self.clone.width(sumWidth);
        this.clone.css({ 'table-layout': 'fixed' });
        self.dom.trigger('forzen.hide', { obj: self, sumWidth: sumWidth });
        self.opts.unforzenHandle && self.opts.unforzenHandle(self);
        if (this.opts.colForzonHead) {
            self.clone.forzenTable('showHeadIndex', this.forzonList);
        }
    }
    forzenTable.prototype._bindEvent = function () {
        var self = this;
        // if (this.opts.colForzonHead) {
        self.$headtable_clone.find('th>.forzontable-th').each(function (key, item) {
            var $that = $(item);
            $that.on('click', function () {
                var index = getThIndex($that.parents('tr:first'), $that.parents('th:first').get(0));
                if (self.__super) {
                    self.__super.forzenCol({ type: 'hide', 'index': index });
                }
            })
        });
        //  }
    }
    forzenTable.prototype.bindEvent = function () {
        var self = this;
        this.opts.fcol && $.each(this.controlList, function (key, item) {
            item.on('click', function () {
                var index = getThIndex(item.parents('tr:first'), item.parents('th:first').get(0));
                self.forzenCol({ type: 'show', 'index': index });
            })
        });
        this.opts.fcol && $.each(this.unForzenList, function (key, item) {
            item.on('click', function () {
                var index = getThIndex(item.parents('tr:first'), item.parents('th:first').get(0));
                self.forzenCol({ type: 'hide', 'index': index });
            })
        });
        ...
        this.scrollElement.on('scroll', function () {
            if (self.opts.fcol) {
                ...
                self.wrap.scrollTop(_scrolltop);
            }
            if (self.opts.fhead) {
                var _scrollleft = $(this).scrollLeft();
                self.$head_wrap.scrollLeft(_scrollleft);
            }
        });
    }
    $.fn.forzenTable = function (opts, _config, _more) {
        if (!(typeof opts == 'string')) {
            opts = $.extend({}, defaults, opts);
            this.each(function () {
                var $this = $(this);
                var _box = new forzenTable(this, opts);
                $this.data().forzenTable = _box;
            });
        } else {
            var res = null;
            this.each(function () {
                var $this = $(this);
                if ($this.data().forzenTable) {
                    // console.log(_config);
                    if (!$this.data().forzenTable[opts]) throw new Error('没有这个方法');
                    res = $this.data().forzenTable[opts](_config, _more);
                    return false;
                }
            });
            return res;
...
(function ($, root, un) {
    ...
    xmsDataTable.prototype.postReload = function () {
        //POST方式请求后台时
    //var formitems = $(this).find('.xmsform-item');
    //var filter = new Xms.Fetch.FilterExpression();
    ...
    //        filter.Conditions.push(condition);
    //        roles_filters.removeAllCondition(name);
    ...
    //});
    //roles_filters.addFilter(new XmsFilter(filter));
    //console.log(roles_filters)
