...
!function (factory) {
    ...
    var toolbtns = [
        //{
        //classname: 'btn btn-link btn-xs', label: '新建', name: 'createBtn', enabled: true, icon: 'glyphicon glyphicon-plus-sign', event: function (e, n, self) {
        //    var url = ORG_SERVERURL + '/entity/create?entityid=' + self.datas.entityId + '&relationshipname=' + self.datas.relationshipname + '&referencedrecordid=' + self.datas.referencedrecordid ;
        ...
        //    $('#createModal').find('.modal-title').html('<span class="glyphicon glyphicon-file"></span> ' + self.datas.entityloclaizedname + ' - ' + (typeof LOC_NEWRECORD == 'undefined' ? '新建' : LOC_NEWRECORD));
        //}, eventtype: 'click'
        //},
        {
            classname: 'btn btn-link btn-xs', label: '插入行', name: 'addRow', enabled: true, icon: 'glyphicon glyphicon-plus', event: function (e, n, self) {
            self.$context.cDatagrid('addRow', { cdatagrid_editer:'new'});
            }, eventtype: 'click'
        }
        //, {
        //    classname: 'btn btn-link btn-xs', label: '保存', name: 'saveGrid', enabled: true, icon: 'glyphicon glyphicon-floppy-disk', event: function (e, n, self) {
        //        self.saveData();
        //    }, eventtype: 'click'
        //}
        , {
    ...
    function entityDatagrid(id,$context) {
        this.prefix = prefix;
        ...
        var searchhtml = ['<table style="position:relative;">',
            ...
            ' <input class="form-control quickly-search-input valid" id="Q" name="Q" placeholder="快速查询" style="width:200px;" type="text" value="">',
    ...
    entityDatagrid.prototype.bindButtonEvent = function (opts) {
        ...
        $.each(this.buttons, function (i,n) {
            self.$buttons.find('button[name="' + n.name + '"]').on(n.eventtype, function (e) {
    ...
    entityDatagrid.prototype.getAndValidDatas = function () {
        ...
        return { isvalid: flag, datas: this.localDatas }
    }
    ...
    entityDatagrid.prototype.setDatas = function (datas) {
        if (datas.queryviews) {
            ...
            this.datas = $.extend({},this.datas,datas);
    ...
    entityDatagrid.prototype.bindEvent = function ($context, evt, ui) {
        var self = this;
        $('button[name=removeRowBtn]', $context).off('click').on('click', null, function (e) {
            var parRow = $(this).parents('tr:first'), index = parRow.index();
            var rowData = self.$plug.cDatagrid('getRowData', index - 1);
            var id = parRow.find('input[name="recordid"]:first').val();
            if (id) {
             //   Xms.Web.Del(id, '/api/data/delete?entityid=' + self.datas.entityId, false, function () {
            ///        
                //   });
                //entitystatus  1:新增，2：修改，3：删除
                rowData.entitystatus = 3;
                self.deleteList.push(rowData);
                self.$plug.cDatagrid('deleteRow', index - 1);
            }
        });
        $('button[name=removeRowBtnLocal]', $context).off('click').on('click', null, function (e) {
            var parRow = $(this).parents('tr:first'), index = parRow.index();
            self.$plug.cDatagrid('deleteRow', index-1);
        });
        $('button[name=searchBtn]', $context).off('click').on('click', null, function (e) {
            self.$plug.cDatagrid('refreshDataAndView');
        });
        $('button[name=clearBtn]', $context).off('click').on('click', null, function (e) {
    ...
    function loadDataGrid($context,datas, isDestroy,opts) {
        ...
        var datagridconfig = {
            ...
            getDataUrl: function (cdatagrid, opts) {
                return ORG_SERVERURL + '/api/data/fetchAndAggregate?entityid=' + datas.entityId + '&queryviewid=' + datas.queryId + '&onlydata=true&pagesize=' + datas.pagesize + '&page=' + cdatagrid.opts.pageModel.page
            },
       ...
       // }
        ...
        datagridconfig.extend = function (datagrid) {
            var extobj = {
                ...
                afterAjax: function (that, objP, DM, PM, FM) {
                    ...
                    $('body').trigger('rendergridview.afterAjax', { $context: $context, that: that, datagridconfig: datagridconfig })
                },
                ...
                filterSendData: function (postData, objP, DM, PM, FM,grid) {
                    ...
                    var ext = { sortby: objP.dataIndx, sortdirection: objP.dir == 'up' ? '0' : '1', pagesize: PM.rPP };
                    if (datas.filter) {
                        ext.filter = datas.filter;
                    }
                    var $searchInput = opts._super.$searcher.find('input[name="Q"]');
        ...
        //设置表格高度
        var parHeight = 0, height = 300, fixHeight = 300;
