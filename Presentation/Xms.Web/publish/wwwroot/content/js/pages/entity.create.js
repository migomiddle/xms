...
!function (factory) {
    ...
    //deps
    var asyncindex = Xms.Form.getTabIsAsync(_form.Panels);
    ...
    var pageWrap_Create = {
        ...
        init: function () {
            $(function () {
                ...
                var $formIframe = $('<iframe id="formIframeContent" src="" name="formIframeContent" class="ifrmae-list" width="100%" frameborder="0" onload="SetWinHeight(this);"  scrolling="no" style="height: 100%; width=100%; visibility: visible; display:none; position:abslute; top:0; bottom:0; left:0; right;0;" ></iframe>');
                        ...
                        $.each(entityDatagirdList.list, function (i,n) {
                            ...
                            if (deldatas && deldatas.length > 0) {
                                $.each(deldatas, function (ii, nn) {
                                    var data = { name: n.datas.entityName, entityid: n.datas.entityId, relationshipname: n.datas.relationshipname, rownumber: i, entitystatus: 3 };
                                    data[n.datas.entityName + 'id'] = nn[n.datas.entityName + 'id']
                                    var temp = {
                                        data: data
                                    };
    ...
    function renderGridView($this, callback) {
        ...
        if ($this.length > 0) {
            ...
            if ($this.attr('data-filter')) {
                data.filter = JSON.parse(decodeURIComponent($this.attr('data-filter')));
            }
        ...
        if (data.theme == 'jqgrid') {
            ...
            Xms.Web.Get(url, function (res) {
                ...
                $this.append($grid);
                var grid = new entityDatagrid(_id, $grid);
