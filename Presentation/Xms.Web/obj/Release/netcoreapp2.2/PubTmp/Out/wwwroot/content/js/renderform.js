...
function insertToAttributeList(e, res) {
    var reid = '';
    Xms.Web.GetJson('/customize/attribute/index', { getall: true, entityid: entityid }, function (data) {
        attributes = data.content.items;
...
function formulaEdit(selected, subgrid_viewid, type) {
                ...
                if (item.type == 'append') {
                    ...
                    $.each(expressArr, function (ii, nn) {
                        ...
                        if (nn.key == "+" || nn.key == "-" || nn.key == "*" || nn.key == "/" || nn.key == "=" || nn.key == "(" || nn.key == ")") {
                            ...
                            if (type == 'edit' && item.viewid) {
                                nn.name = getEntityCnName(subGridCache[item.viewid].alldatas.content, temp);
                            } else {
                                nn.name = getEntityCnName(subGridCache[subgrid_viewid].alldatas.content, temp);
                            }
                        }
    ...
    formula_entitys.children("option").on("click", function () {
        var $this = $(this);
        var viewid = $('#subgrid-viewid>option:selected').val();
        if ($("#subgrid-entityedit>option:selected").val() == -1) return false;
                ...
                if (typeof lookuplistCache[$this.val() + viewid] == 'undefined') {
                    ...
                    $.each(lookuplistCache[$this.val() + viewid], function (key, item) {
                        if (item) {
                            ...
                            var tarvcn = getEntityCnName(subGridCache[viewid].alldatas.content, tarv);
                            var sourvcn = getEntityCnName(subGridCache[reentityid + "&getall=true"].alldatas.content.items, sourv);
...
function subgridGetAttributes(entityid, callback, type, isall) {
    ...
    if (entityid && entityid != "") {
        if (typeof subGridCache[entityid + getall] == 'undefined') {//缓存中是否已有数据
            Xms.Web.GetJson('/customize/attribute/index?entityid=' + entityid + getall, null, function (alldatas) {
                /*缓存数据*/
...
function editTab() {
    ...
    for (var i = 0; i < eventlist.length; i++) {
        if (eventlist[i].Attribute.toLowerCase() == selected.attr('data-name').toLowerCase()) {
            target.find('#tabEvent').append('<tr onclick="chooseRowInfo(this)"><td><select class="scriptlist">' + scriptselect.join('') + '</select></td><td><select><option value="onstatechange">onstatechange</option></select></td><td><input type="text" value="' + eventlist[i].JsAction + '" /></td></tr>');
...
function editField() {
    ...
    if (controltype == 'lookup' || controltype == 'owner' || controltype == 'customer') {
        ...
        if (entityid != '') {
            ...
            var html = [];
            Xms.Web.GetJson('/customize/QueryView/index?getall=true&EntityId=' + entityid + '', null, function (data) {
                console.log(data.content)
    ...
    for (var i = 0; i < eventlist.length; i++) {
        if (eventlist[i].Attribute.toLowerCase() == selected.attr('data-name').toLowerCase()) {
            target.find('#fieldEvent').append('<tr class="eventitem" onclick="chooseRowInfo(this)"><td><select class="scriptlist">' + scriptselect.join('') + '</select></td><td><select><option value="onchange">onchange</option></select></td><td><input type="text" value="' + eventlist[i].JsAction + '" /></td></tr>');
...
function changeGetEntityName(obj, callback, target, filter, isrequire) {
    ...
    Xms.Web.GetJson('/customize/attribute/index?getall=true&entityid=' + entityid + '', null, function (data) {
        var html = [];
        var repdatas = data.content.items;
...
function editLabel() {
    ...
    for (var i = 0; i < eventlist.length; i++) {
        if (eventlist[i].Attribute.toLowerCase() == selected.attr('data-name').toLowerCase()) {
            target.find('#fieldEvent').append('<tr onclick="chooseRowInfo(this)"><td><select class="scriptlist">' + scriptselect.join('') + '</select></td><td><select><option value="onchange">onchange</option></select></td><td><input type="text" value="' + eventlist[i].JsAction + '" /></td></tr>');
            target.find('#fieldEvent').find('tr:last').find('td:eq(0)').find('option[value="' + eventlist[i].JsLibrary + '"]').attr('selected', "selected");
            target.find('#fieldEvent').find('tr:last').find('td:eq(1)').find('option[value="' + eventlist[i].Name + '"]').attr('selected', "selected");
        }
    }
