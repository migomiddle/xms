...
Xms.Web = {
    Queue: new function () {
        ...
        this.remove = function (key) {
            index--;
            if (index == 0) {
                $(document).trigger("ajaxQueueEmpty");
                $(document).trigger("ajaxOtherFunc");
            }
        }
    }
    ...
    , Post: function (url, data, refresh, onsuccess, onerror, __async, istip,extopts) {
        ...
        var dtd = $.Deferred();
        // Xms.Web.Console('postMethod.caller', arguments.callee.caller);
        //  Xms.Web.Console('purecmsPost', { url: url, data: data });
    ...
    , GetJson: function (url, data, onsuccess, onerror, __async, type, issyn) {
        ...
        // Xms.Web.Console('getjsonMethod.caller', arguments.callee.caller);
        if (issyn) {
            isAsync = false;
        }
        type = type || 'GET';
        //Xms.Web.Console('purecmsGetjson', { url: url, data: data });
        var dtd = $.Deferred();
        if (url.indexOf(ORG_SERVERURL) !== 0) {
            url = ORG_SERVERURL + (url.indexOf('/') == 0 ? "" : "/") + url;
        }
    ...
    , Get: function (url, onsuccess, onerror) {
        ...
        var dtd = $.Deferred();
        // Xms.Web.Console('getMethod.caller', arguments.callee.caller);
        //Xms.Web.Console('purecmsGet', { url: url });
        if (url.indexOf(ORG_SERVERURL) !== 0) {
            url = ORG_SERVERURL + (url.indexOf('/') == 0 ? "" : "/") + url;
        }
        $.ajax({
    ...
    , Form: function (target, onsuccess, onerror, beforesubmit, setting) {
        ...
        target.find("input.required:not('.lookup')").each(function (i, n) {
            ...
            //$(this).parents('.form-group').find(".control-label").append('<span class="text-danger pull-right">*</span>');
            //validRules.push('"' + name + '":{"required":true}');
            validrule.add(name, { "required": true });
        });
        // console.log("validRules:" + validRules)
        //添加lookup类型判断
        //console.log("input.lookup:", target.find("input.lookup"))
        ...
        target.find("input[data-val-length-max]").each(function (i, n) {
            ...
            //validRules.push('"' + name + '":{"maxlength":' + $(this).prop("maxlength") + '}');
        ...
        target.find("input[data-val-length-min]").each(function (i, n) {
            ...
            // validRules.push('"' + name + '":{"minlength":' + $(this).prop("minlength") + '}');
        ...
        target.find("input[data-val-email]").each(function (i, n) {
            ...
            // validRules.push('"' + name + '":{"email":true}');
        ...
        target.find("input[data-val-min]").each(function (i, n) {
            ...
            var name = $(this).prop('name');
            //validRules.push('"' + name + '":{"min":' + $(this).attr("data-val-min") + '}');
        ...
        target.find("input[data-val-max]").each(function (i, n) {
            ...
            var name = $(this).prop('name');
            //validRules.push('"' + name + '":{"max":' + $(this).attr("data-val-max") + '}');
        ...
        target.find("input[data-range]").each(function (i, n) {
            ...
            // validRules.push('"' + name + '":{"range":[' + a[0] + ',' + a[1] + ']}');
        ...
        target.find("input[data-rangelength]").each(function (i, n) {
            ...
            // validRules.push('"' + name + '":{"rangelength":[' + a[0] + ',' + a[1] + ']}');
        ...
        target.find("input[data-type=int]").each(function (i, n) {
            ...
            var name = $(this).prop('name');
            //validRules.push('"' + name + '":{"isInt":true}');
        ...
        target.find("input[data-type=float]").each(function (i, n) {
            ...
            var name = $(this).prop('name');
            //validRules.push('"' + name + '":{"digits":true}');
        ...
        target.find("input[data-type=number],input[data-val-number]").each(function (i, n) {
            ...
            var name = $(this).prop('name');
            // validRules.push('"' + name + '":{"number":true}');
        ...
        target.find("input[data-type=date]").each(function (i, n) {
            ...
            var name = $(this).prop('name');
            // validRules.push('"' + name + '":{"date":true}');
        ...
        target.find("input[data-equalto]").each(function (i, n) {
            ...
            var name = $(this).prop('name');
            // validRules.push('"' + name + '":{"equalTo":"' + $(this).attr("data-equalto") + '"}');
        ...
        var valiSetting = {
            ...
            //    var count = this.numberOfInvalids();
            //    if (count > 0) {
            //        //Xms.Web.Alert(false, "有" + count + "个错误未处理");
            //        $.messager.popup("有" + count + "个错误未处理");
            //    }
                ...
                if (ajaxsubmit != "" && ajaxsubmit == "false") {
                    ...
                    jQuery(form).ajaxSubmit({
                        ...
                        beforeSubmit: function () {
                            //Xms.Web.Alert(false, '请重新<a href="/account/signin" target="_blank">登录</a>(新窗口打开)，登录成功后回到本页面点击确定即可'
                            //    , function () {
                            //    });
                            //return false;
                            if (typeof (beforesubmit) == "function") {//提交前方法
    ...
    , OpenDialog: function (url, callback, data, onopened) {
        var link = url;// + (callback ? op + "callback=" + callback : "");
        ...
        //if (callback && typeof (callback) == 'function') Xms.Web.Dialog.callback.push({});
        //if (!data) data = new Object();
        //if (callback) data.callback = typeof (callback) == 'function' ? callback.toString() : callback;
        ...
        Xms.Web.LoadPage(link, data, function (response) {
            ...
            if ($.fn.insertResizeBox) {
                if (container.find('.modal:first').length > 0) {
                    //container.find('.modal-dialog:first').insertResizeBox({
                    //    drag: function (type, event, ui, tempOffsetX, tempOffsetY) {
                    //        var self = this;
                    //        self.dom.children('.modal-content:first').css({ 'height': tempOffsetY });
                    //    },
                    //    isActive:[0,0,0,0]//没有激活任何调整宽高的  设为1即可添加
                    //});
                }
            }
    ...
    , Console: function () {
        if ($.getUrlParam && $.getUrlParam('xmsdebug')) {
    ...
    , callParentMethod: function (method, arg1, arg2, arg3, arg4, arg5, arg6) {
        if (parent && parent[method]) {
            parent[method](document.body, arg1, arg2, arg3, arg4, arg5, arg6);
        }
    }
...
Xms.Ajax = new function () {
    ...
    this.send = function (opts) {
        ...
        if (~opts.contentType.indexOf('json')) {
            opts.data = JSON.stringify(opts.data);
        }
        $.ajax({
            type: opts.type,
