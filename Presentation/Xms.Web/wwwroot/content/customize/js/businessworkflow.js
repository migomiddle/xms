...
$(function () {
    ...
    $('button[type="submit"]').off('click').on('click', function (e) {
        ...
        var url = 'createBusinessFlow';
        if (PAGE_TYPE == 'EDIT') {
            url = 'EditBusinessFlow';
            ...
            if (PAGE_TYPE == 'CREATE') {
                if (res.IsSuccess) {
                    Xms.Web.Alert(true, res.Content, function () {
                        location.href = '/customize/flow/editbusinessflow?id=' + res.Extra.id;
                    });
                } else {
    ...
    function itemModel(name, stagetype, parid, processstageid) {
        ...
        this.stageHandler = function (obj, e) {
            ...
            if (e.stopPropagation) { e.stopPropagation(); }
            var $target = $(target);
            var Par = $target.parents('tr.step-item');
            if (Par.length > 0) {
                Par.siblings('tr').removeClass('active').end().addClass('active');
                self._super.activeStage(self);
            }
            return true;
        }
    ...
    function initPage() {
        ...
        //  console.log(datas);
        var entitydatas = Xms.Web.PageCacheData['businessflow']['allentity'].data.content.items;
        var count = 0;
        var ischange = false;
        var currentEntityid = '';
                    ...
                    if (rel == "") {//第一个实体的数据
                        ...
                        ; (function (k, kentityid) {
                            ...
                            getAttributeByEntityId(kentityid, function () {
                                var stagesData = Xms.Web.PageCacheData['businessflow']['attribute' + kentityid].data.content.items;
                                for (var ii in k) {
                        ...
                        getEntityRelation(curentityid, function () {
                            ...
                            ; (function (k, kentityid) {
                                getAttributeByEntityId(jj, function () {
                                    var stagesData = Xms.Web.PageCacheData['businessflow']['attribute' + jj].data.content.items;
                                    for (var ii in k) {
                                        if (k.hasOwnProperty(ii) && ii != 'relationshipname') {
                                            var nn = k[ii];
                                            $.each(nn, function (iii, nnn) {
                                                ...
                                                var stepArr = JSON.parse(stepStr);
                                                $.each(stepArr, function (key, item) {
                                                    Xms.Web.PageCacheData['businessflow']['attribute' + jj].data.content.items;
                                                    var attrname = getEntityData(stagesData, 'name', item.attributename, 'localizedname');
                                                    var displayname = item.displayname;
                                                    var isrequired = item.isrequired;
                                                    var attrvalue = item.attributename;
    ...
    function entitychange() {
        $('#EntitySel').on('change', function () {
            Xms.Web.Confirm('更改实体', '更换实体会清除已设置的数据，是否更改？', function () {
                ...
                gApp.entityList.addEntity(name, value, key);
            });
        });
    }
