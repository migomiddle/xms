...
(function ($) {
        ...
        if (objP) {
            ...
            if (that.totalVisibleRows === 0) {
                ...
                if (pqpanes.h && pqpanes.v) {
                    ...
                    if (pqpanes.v) {
                        if (freezeCtrl) {
                            $cont[0].innerHTML = ["<div class='pq-grid-cont-inner pq-grid-cont-inner-freezeleft' ", style, " >", strTbl, "</div>", "<div class='pq-grid-cont-inner pq-grid-cont-inner-right' >", strTbl, "</div>", "<div class='pq-grid-cont-inner' >", strTbl, "</div>"].join("")
                        } else {
                            $cont[0].innerHTML = ["<div class='pq-grid-cont-inner pq-grid-cont-inner-freezeleft' ", style, " >", strTbl, "</div>", "<div class='pq-grid-cont-inner' >", strTbl, "</div>"].join("")
                        }
                    } else {
                        if (pqpanes.h) {
                ...
                if (pqpanes.v) {
                    if (freezeCtrl) {
                        ...
                        $cont_l.on("scroll", function (evt) {
                            ...
                            this.scrollLeft = 0
                            if ($cont_or.length > 0) {
                                var cont_or = $cont_or.get(0)
                                // $cont_or.on("scroll", function (evt) {
                                cont_or.scrollTop = iMS.getScrollTop();
                                //  cont_or.scrollLeft = iMS.getScrollLeft()
                                // });
                            }
                        });
        ...
        if ($tbl && $tbl.length) {
            ...
            if (pqpanes.h && pqpanes.v) {
                ...
                if (pqpanes.v) {
                    ...
                    if (freezeCtrl) {
                        var _le = calcWidthCols_ctrl.call(that, freezeCols, initH);
                        $cont_or.css({
                            left: 'auto',
                            right: 0,
                            width: _le,
    ...
    _pGenerateView._getPrimaryKey = function () {
        var res = null,
            that = this,
            options = this.options;
        $.each(options.colModel, function (key, item) {
    ...
    _pGenerateView._renderCell = function (objP, _dataCell) {
        ...
        if (_dataCell != undefined) {
            ...
            if (type == "checkBoxSelection") {
                var prikey = this._getPrimaryKey();
                if (prikey) {
                    dataCell = "<input type='checkbox' " + (cellData ? "checked='checked'" : "") + " name='" + dataIndx + "' value='" + rowData[prikey.name] + "'  />"
                } else {
    ...
    _pGenerateView._generateRow = function (rowData, rowIndx, CM, buffer, objP, lastFrozenRow, nestedshow) {
        ...
        if (CM && CM.length > 0 && CM[0].xmsLineEvent) {
            var tr_html = CM[0].xmsLineEvent(rowData, rowIndx, CM, row_cls, attr);
            if (tr_html && tr_html != '') {
                buffer.push(tr_html);
            } else {
    ...
    if (typeof PAGEDEFAULT_PAGESIZE != '') {
        fn.options.pageModel.rPPOptions.push(PAGEDEFAULT_PAGESIZE * 1);
        fn.options.pageModel.rPPOptions.sort(function (a, b) {
            return a - b;
        });
        fn.options.pageModel.rPPOptions = fn.options.pageModel.rPPOptions.unique();
    }
    fn._regional = {
        strAdd: "Add",
        strDelete: "Delete",
    ...
    fn.remoteRequest = function (objP) {
        ...
        if (!objP.dataIndx && !objP.dir && this.oldSortAttrbutename) {
            objP.dataIndx = this.oldSortAttrbutename;
            objP.dir = this.oldSortDir;
        }
        this.loading = true;
        this.showLoading();
        var objPostdata = toQueryParams.call(url);
        if (iSort) {
            var sorters = iSort.sorters;
            var dir = '0';
            for (var i = 0; i < sorters.length; i++) {
                var sorter = sorters[i],
                    dataIndx = sorter.dataIndx;
                if (objP.dataIndx == dataIndx) {
                    dir = sorter.dir;
                    objP.dir = dir;
                }
            }
        ...
        if (dataIndx) {
            this.oldSortAttrbutename = dataIndx;
            this.oldSortDir = dir;
        }
        if (DM.isJsonAjax) {
            var url = getDataUrlApi.call(url);
            if (DM.filterSendData) {
                if (this.oldSortAttrbutename) {
                    objP.dataIndx = this.oldSortAttrbutename;
                    objP.oldSortDir = this.oldSortDir;
                }
                objPostdata = DM.filterSendData(objPostdata, objP, DM, PM, FM, this);
            }
            if (DM.method == 'POST') {
                var __datas = JSON.stringify(objPostdata);
            } else {
                __datas = objPostdata;
            }
        } else {
        ...
        if (DM.beforeAjax) {
            var isnext = DM.beforeAjax(that, objP, DM, PM, FM);
            if (!isnext) {
                return false;
            }
        }
        // console.log(__datas, __api);
        this.xhr = $.ajax({
            url: url,
            ...
            beforeSend: function (jqXHR, settings) {
                if (typeof DM.beforeSend == "function") {
                    return DM.beforeSend.call(that.element[0], jqXHR, settings, that)
                }
            },
    ...
    fn._digestData = function (ui) {
        ...
        for (var i = 0; i < rowListLen; i++) {
            ...
            for (var dataIndx in newRow) {
                ...
                if (column) {
                    if (rowCheckEditable && column.editable != null && column.isDelete && (that.isEditableCell({
                        rowIndx: rowIndx,
                        colIndx: colIndx,
                        dataIndx: dataIndx
                    }) === false)) {
    ...
    fn._editCell = function (objP) {
        ...
        if (rowData['__xms_pqgrid_editabled_' + dataIndx] === false) {
            return false;
        }
                ...
                if (edtype == "textarea" || edtype == "select" || edtype == "textbox") {
                    if (edtype == "textarea") {
                        ...
                        if (edtype == "select") {
                            ...
                            inp = "<input class='form-control " + cls2 + "' " + attr + " " + style + " type=text name='" + dataIndx + "' />"
                        }
                    }
    ...
    var calcWidthCols_ctrl = function (colIndx1, colIndx2, _direct) {
        var wd = 0,
            o = this.options,
            columnBorders = o.columnBorders,
            cbWidth = (columnBorders ? 1 : 0),
        ...
        if (_direct) {
            for (var i = this.colModel.length - 1; i <= this.colModel.length; i++) {
                var column = CM[i];
                if (column && !column.hidden) {
                    if (!column._width) {
                        throw ("assert failed")
    ...
    $.paramquery.pqgrid.calcWidthCols_ctrl = calcWidthCols_ctrl;
...
(function ($) {
    ...
    _p.createHeader = function () {
        ...
        for (var row = 0; row < depth; row++) {
            ...
            if (row == 0 && numberCell.show) {
                buffer.push(["<td pq-col-indx='-1' class='pq-grid-number-col' rowspan='", depth, "'>", "<div class='pq-td-div' data-fieldname='" + (numberCell.dataIndx ? numberCell.dataIndx : "&nbsp;") + "'>", numberCell.title ? numberCell.title : "&nbsp;", "</div>", "</td>"].join(""))
            }
            for (var col = 0; col <= finalH; col++) {
                if (col < initH && col >= freezeCols && virtualX) {
                    col = initH;
        ...
        $header.find('.pq-td-div').click(function (evt) {
    ...
    _p._onHeaderClick = function (evt) {
        ...
        return self._onHeaderCellClick(colIndx, evt, obj)
    };
    _p.createHeaderCell = function (row, col, headerCells, buffer, const_cls, freezeCols, initH, depth) {
        ...
        buffer.push(["<td ", colIndx, " ", colIndxDD, " ", rowIndxDD, " ", " class='", 'datatable-header-td ', cls, "' rowspan=", column.rowSpan, " colspan=", colSpan, ">", "<div class='pq-td-div' data-fieldname='" + column.dataIndx + "'>", title, "<span class='pq-col-sort-icons'>&nbsp;</span></div><span class='forzen-ctrl glyphicon glyphicon-pushpin' title='冻结该列以前'></span><span class='datatable-filter-list'>&nbsp;</span></td>"].join(""))
    };
    _p._onHeaderCellClick = function (colIndx, evt, obj) {
        var that = this.that,
            column = that.colModel[colIndx],
            o = that.options,
            dataIndx = column.dataIndx;
    ...
    _p.refreshHeaderSortIcons = function () {
        ...
        $header.find(".pq-col-sort-icons").removeClass("glyphicon glyphicon-sort-by-attributes glyphicon-sort-by-attributes-alt");
        for (var i = 0; i < sorters.length; i++) {
            var sorter = sorters[i],
                dataIndx = sorter.dataIndx,
                colIndx = that.getColIndx({
                ...
                cls2 = "glyphicon glyphicon-sort-by-attributes" + (dir == "up" ? "" : "-alt");
            $header.find(".pq-grid-col-leaf[pq-col-indx=" + colIndx + "]").addClass(addClass);
            $header.find(".pq-grid-col-leaf[pq-col-indx=" + colIndx + "] .pq-col-sort-icons").addClass(cls2)
        }
    };
...
(function ($) {
    ...
    _pHeaderSearch.createDOM = function (buffer, td_const_cls) {
        ...
        if (numberCell.show) {
            buffer.push(["<td pq-col-indx='-1' class='pq-grid-number-col' rowspan='1'>", "<div class='pq-td-div'>&nbsp;</div><span class='datatable-filter-list'>&nbsp;</span></td>"].join(""))
        }
        for (var i = 0; i <= finalH; i++) {
            if (i < initH && i >= freezeCols && virtualX) {
                i = initH;
            ...
            if (filter) {
                ...
                buffer.push(["<td class='", td_cls, "'><div class='pq-td-div' data-fieldname='" + dataIndx + "' >", "", strS, "</div><span class='datatable-filter-list'>&nbsp;</span></td>"].join(""))
            } else {
                buffer.push(["<td class='", td_cls, "'><div class='pq-td-div'  >", "&nbsp;", "</div><span class='datatable-filter-list'>&nbsp;</span></td>"].join(""))
            }
        }
        buffer.push("</tr>")
    };
    _pHeaderSearch._input = function (dataIndx, value, cls, style, attr) {
        return ['<input value="', value, "\" name='", dataIndx, "' type=text style='" + style + "' class='form-control " + cls + "' " + attr + " />"].join("")
    };
    _pHeaderSearch._textarea = function (dataIndx, value, cls, style, attr) {
        return ["<textarea name='", dataIndx, "' style='" + style + "' class='form-control " + cls + "' " + attr + " >", value, "</textarea>"].join("")
    };
    ...
    _pGenerateView._generateTitleRow = function (GM, rowObj, buffer, lastFrozenRow) {
        ...
        buffer.push("<td class='pq-grid-cell' colSpan='100' >", "<div class='pq-td-div' style='margin-left:", (groupLevel * 16), "px;'>", "<span class='ui-icon ", icon, "'></span>", groupTitle, "</div><span class='datatable-filter-list'>&nbsp;</span></td>");
        buffer.push("</tr>")
    };
    ...
    fnTB._create = function () {
        ...
        for (var i = 0, len = items.length; i < len; i++) {
            var item = items[i],
                ...
                $ctrl;
            if (type == "textbox") {
                $ctrl = $("<input type='text' class='form-control " + cls + "' " + attr + " " + style + ">").appendTo(element)
            } else {
                if (type == "checkbox") {
                ...
                for (var j = 0; j < listeners.length; j++) {
                    ...
                    (function (_listener) {
                        for (var event in _listener) {
                            $ctrl.bind(event, function (e) {
                                _listener[event].call(this, e, self.options._super, self.options._super.element)
                            })
                        }
                    })(listener);
                }
...
(function ($) {
    ...
    _pCheckBC.onHeaderClick = function (evt) {
        ...
        if (finalChk) {
            ...
            that.$cont.find('.pq-grid-cont-inner:first').find("td[pq-col-indx=" + colIndx + "] input").prop("checked", true);
    ...
    _pCheckBC.rowCheck = function (evt, ui) {
        var that = this.that,
            rowData = ui.rowData,
    ...
    _pCheckBC.rowUnCheck = function (evt, ui) {
        var that = this.that,
            rowData = ui.rowData,
            inpT = $(evt.originalEvent.target).is("input") ? true : false,
            rowIndx = ui.rowIndx,
